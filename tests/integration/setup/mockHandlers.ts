...
// import { MESSAGES } from '@shared-constants'; // Will be enabled when path mapping is configured

// Temporary mock messages for testing until @shared-constants is available
const MOCK_MESSAGES = {
  ERRORS: {
...
export const handlers = [
  ...
  http.post('/auth/v1/token', async ({ request }) => {
    ...
    return HttpResponse.json(
      { error: MOCK_MESSAGES.ERRORS.INVALID_CREDENTIALS },
      { status: 400 }
    );
  }),
  ...
  http.get('/auth/v1/user', ({ request }) => {
    ...
    return HttpResponse.json(
      { error: MOCK_MESSAGES.ERRORS.UNAUTHORIZED },
      { status: 401 }
    );
  }),
  ...
  http.get('/rest/v1/transactions-error', () => {
    return HttpResponse.json(
      { error: MOCK_MESSAGES.ERRORS.NETWORK_ERROR },
      { status: 500 }
    );
  }),
];
...
export const errorHandlers = [
  ...
  http.post('/rest/v1/transactions', () => {
    return HttpResponse.json(
      { error: MOCK_MESSAGES.ERRORS.VALIDATION_FAILED },
      { status: 400 }
    );
  }),
]; 