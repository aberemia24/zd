  ...
  devtools(
    (set, get) => {
      ...
      return {
        ...
        setFilterType: (type) => {
          set({ filterType: type, offset: 0, lastUpdated: new Date() }, false, 'setFilterType');
          logAction('Filter type changed', { type });
        },
        
        setFilterCategory: (cat) => {
          set({ filterCategory: cat, offset: 0, lastUpdated: new Date() }, false, 'setFilterCategory');
          logAction('Filter category changed', { category: cat });
        },
        
        setFilterSubcategory: (subcat) => {
          set({ filterSubcategory: subcat, offset: 0, lastUpdated: new Date() }, false, 'setFilterSubcategory');
          logAction('Filter subcategory changed', { subcategory: subcat });
        },
        
        setDateFrom: (date) => {
          set({ dateFrom: date, offset: 0, lastUpdated: new Date() }, false, 'setDateFrom');
          logAction('Date from changed', { dateFrom: date });
        },
        
        setDateTo: (date) => {
          set({ dateTo: date, offset: 0, lastUpdated: new Date() }, false, 'setDateTo');
          logAction('Date to changed', { dateTo: date });
        },
        
        setAmountMin: (amount) => {
          set({ amountMin: amount, offset: 0, lastUpdated: new Date() }, false, 'setAmountMin');
          logAction('Amount min changed', { amountMin: amount });
        },
        
        setAmountMax: (amount) => {
          set({ amountMax: amount, offset: 0, lastUpdated: new Date() }, false, 'setAmountMax');
        ...
        setTotalItems: (total) => {
          set({ totalItems: total, lastUpdated: new Date() }, false, 'setTotalItems');
        ...
        nextPage: () => {
          ...
          if (newOffset < totalItems) {
            set({ offset: newOffset, lastUpdated: new Date() }, false, 'nextPage');
            logAction('Next page', { newOffset, page: Math.floor(newOffset / limit) + 1 });
          }
        },
        ...
        prevPage: () => {
          ...
          logAction('Previous page', { newOffset, page: Math.floor(newOffset / limit) + 1 });
        },
        ...
        loadFromURL: (searchParams: URLSearchParams) => {
          ...
          const offset = parseInt(searchParams.get('offset') || String(PAGINATION.DEFAULT_OFFSET));
          ...
          logAction('Filters loaded from URL', { 
            type, category, subcategory, dateFrom, dateTo, 
            amountMin, amountMax, searchText, limit, offset, sort 
          });
        },
        ...
        getURLSearchParams: () => {
          ...
          if (limit !== PAGINATION.DEFAULT_LIMIT) params.set('limit', limit.toString());
          if (offset !== PAGINATION.DEFAULT_OFFSET) params.set('offset', offset.toString());
...
export const useFiltersState = () => useTransactionFiltersStore(state => ({
...
export const usePaginationState = () => useTransactionFiltersStore(state => ({
  limit: state.limit,
  offset: state.offset,
  sort: state.sort,
