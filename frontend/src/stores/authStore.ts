    ...
    persist(
      (set, get) => {
        ...
        return {
          ...
          login: createAuthAction('login', async (email: string, password: string) => {
            ...
            storeLogger.info(STORE_NAME, 'Login successful', { userId: result.user?.id });
          }),

          register: createAuthAction('register', async (email: string, password: string) => {
            ...
            storeLogger.info(STORE_NAME, 'Registration successful', { userId: result.user?.id });
          }),
          ...
          setUser: (user: AuthUser | null) => {
            ...
            storeLogger.info(STORE_NAME, 'User set manually', { userId: user?.id });
          },
...
supabaseAuthService.onAuthStateChange((event, session) => {
  ...
  storeLogger.info(STORE_NAME, 'Auth state change', { event, sessionExists: !!session });
