...
export const supabaseService = {
  ...
  async fetchTransactions(
    ...
    filters: Partial<
      Pick<TransactionValidated, "type" | "category" | "recurring">
    > & {
  ...
  validateCategoryAndSubcategory(
    ...
    console.debug(
      `[validateCategoryAndSubcategory] Validare: category=${category}, subcategory=${subcategory}`,
    );
    ...
    if (!Object.keys(CATEGORIES).includes(category)) {
      console.warn(
        `Categorie "${category}" nu există în lista predefinită. Poate fi o categorie personalizată.`,
      );
    ...
    if (subcategory) {
      console.debug(
        `[DEBUG] Subcategorie "${subcategory}" acceptată pentru categoria "${category}" (validare delegată backend-ului).`,
      );
    }
  ...
  async createTransaction(
    ...
    this.validateCategoryAndSubcategory(
      payload.category,
