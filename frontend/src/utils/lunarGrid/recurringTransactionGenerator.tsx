    ...
    case "weekly":
      ...
      if (frequency.dayOfWeek !== undefined) {
        const daysUntilTarget =
          (frequency.dayOfWeek - nextDate.getDay() + 7) % 7;
        const daysToAdd =
          daysUntilTarget === 0 ? 7 * frequency.interval : daysUntilTarget;
...
export function generateRecurringTransactions(
  ...
  const effectiveStart = new Date(
    Math.max(templateStart.getTime(), periodStart.getTime()),
  );
  const effectiveEnd = new Date(
    Math.min(templateEnd.getTime(), periodEnd.getTime()),
  );
...
export function detectConflicts(
  ...
  const manualTransactionsMap = new Map<
    string,
    (typeof existingTransactions)[0]
  >();
  ...
  if (conflicts.length > 0) {
    ...
    const significantDifferences = conflicts.filter(
      (c) => Math.abs(c.amountDifference) > 100,
    );
...
export function validateRecurringTemplate(
  ...
  if (!template.frequency) {
    ...
    if (
      template.frequency.type === "weekly" &&
      template.frequency.dayOfWeek !== undefined
    ) {
      if (
        template.frequency.dayOfWeek < 0 ||
        template.frequency.dayOfWeek > 6
      ) {
        errors.push(
          "Ziua săptămânii trebuie să fie între 0 (Duminică) și 6 (Sâmbătă)",
        );
      }
    }

    if (
      template.frequency.type === "monthly" &&
      template.frequency.dayOfMonth !== undefined
    ) {
      if (
        template.frequency.dayOfMonth < 1 ||
        template.frequency.dayOfMonth > 31
      ) {
    ...
    if (
      template.frequency.type === "yearly" &&
      template.frequency.monthOfYear !== undefined
    ) {
      if (
        template.frequency.monthOfYear < 1 ||
        template.frequency.monthOfYear > 12
      ) {
  ...
  if (template.amount && template.amount > 100000) {
    warnings.push(
      "Suma este foarte mare (>100,000 RON) - verificați dacă este corectă",
    );
  }
  ...
  if (template.frequency && errors.length === 0) {
    try {
      ...
      estimatedImpact.totalTransactionsNextYear =
        monthlyTransactions.length * 12;
    } catch (error) {
  ...
  switch (frequency.type) {
    case "daily":
      return frequency.interval === 1
        ? "Zilnic"
        : `La fiecare ${frequency.interval} zile`;
    ...
    case "weekly":
      ...
      const dayName =
        frequency.dayOfWeek !== undefined ? dayNames[frequency.dayOfWeek] : "";
      return frequency.interval === 1
        ? `Săptămânal${dayName ? ` (${dayName})` : ""}`
        : `La fiecare ${frequency.interval} săptămâni${dayName ? ` (${dayName})` : ""}`;

    case "monthly":
      const monthlyDayText = frequency.dayOfMonth
        ? ` în ziua ${frequency.dayOfMonth}`
        : "";
      return frequency.interval === 1
        ? `Lunar${monthlyDayText}`
        : `La fiecare ${frequency.interval} luni${monthlyDayText}`;
    case "yearly":
      const monthNames = [
        "Ianuarie",
        "Februarie",
      ...
      const monthText = frequency.monthOfYear
        ? ` în ${monthNames[frequency.monthOfYear - 1]}`
        : "";
      const yearlyDayText = frequency.dayOfMonth
        ? ` ziua ${frequency.dayOfMonth}`
        : "";
      return frequency.interval === 1
        ? `Anual${monthText}${yearlyDayText}`
