...
import { calculateDailyBalances, calculateWithValidation } from '../financialCalculations';
...
describe('Financial Calculations', () => {
  ...
  describe('calculateDailyBalances', () => {
    test('INCOME increases available balance', () => {
      const transactions = new Map([
        ['2024-01-01', [createMockTransaction(TransactionType.INCOME, 500, '2024-01-01')]]
      ]);
    ...
    test('EXPENSE decreases available balance', () => {
      const transactions = new Map([
        ['2024-01-01', [createMockTransaction(TransactionType.EXPENSE, 200, '2024-01-01')]]
      ]);
    ...
    test('SAVING moves money from available to savings', () => {
      const transactions = new Map([
        ['2024-01-01', [createMockTransaction(TransactionType.SAVING, 300, '2024-01-01')]]
      ]);
    ...
    test('handles sequential days correctly', () => {
      const transactions = new Map([
        ['2024-01-01', [createMockTransaction(TransactionType.INCOME, 500, '2024-01-01')]],
        ['2024-01-02', [createMockTransaction(TransactionType.EXPENSE, 200, '2024-01-02')]]
      ]);
    ...
    test('handles string amounts correctly', () => {
      ...
      expect(result[0].availableBalance).toBe(1500.50);
      expect(result[0].breakdown.dailyIncome).toBe(500.50);
    });
  });
  ...
  describe('calculateWithValidation', () => {
    test('validates negative starting balance', () => {
      ...
      expect(result.errors).toContain('Balanta de pornire nu poate fi negativă');
    });
    ...
    test('validates negative transaction amounts', () => {
      ...
      expect(result.hasErrors).toBe(true);
      expect(result.errors).toContain(`Suma negativă în tranzacția ${transactionWithNegativeAmount.id}`);
    });
