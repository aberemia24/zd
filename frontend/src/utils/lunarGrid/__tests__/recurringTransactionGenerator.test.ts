...
describe('Recurring Transaction Generator', () => {
  ...
  // =============================================================================
    ...
    describe('detectConflicts', () => {    const recurringTransactions = [      {        id: 'recurring-1-2024-01-01',        userId: 'user-1',        amount: 1000,        type: TransactionType.EXPENSE,        categoryId: 'category-1',        subcategoryId: 'subcategory-1',        date: '2024-01-01',        isRecurring: true as const,        recurringTemplateId: 'template-1',        isOverridden: false,        overrideTransactionId: undefined      },      {        id: 'recurring-2-2024-01-02',        userId: 'user-1',        amount: 500,        type: TransactionType.EXPENSE,        categoryId: 'category-2',        date: '2024-01-02',        isRecurring: true as const,        recurringTemplateId: 'template-2',        isOverridden: false,        overrideTransactionId: undefined      }    ];
    ...
    test('marks recurring transactions as overridden', () => {
      ...
      const overriddenTransaction = recurringTransactions.find(tx => tx.date === '2024-01-01');
    ...
    test('provides resolution suggestions', () => {
      ...
      expect(conflicts.resolutionSuggestions[0]).toContain('conflicts detected');
    });
    ...
    test('handles no conflicts scenario', () => {
      ...
      const conflicts = detectConflicts(recurringTransactions, noConflictTransactions);
  ...
  describe('resolveConflicts', () => {
    ...
    test('filters out conflicted transactions', () => {
      const resolved = resolveConflicts(recurringTransactions, existingTransactions);
  ...
  describe('validateRecurringTemplate', () => {
    ...
    test('validates frequency constraints', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors.some(error => error.includes('săptămânii'))).toBe(true);
    });
    ...
    test('provides warnings for edge cases', () => {
      ...
      expect(result.warnings.some(warning => warning.includes('mare'))).toBe(true);
    });
    ...
    test('calculates estimated impact', () => {
      ...
      expect(result.estimatedImpact.totalTransactionsNextYear).toBeGreaterThan(0);
    });
  });
