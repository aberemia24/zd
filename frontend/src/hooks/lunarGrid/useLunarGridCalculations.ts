...
import {   calculateDailyBalances as performDailyBalances,   recalculateFromDate as performRecalculateFromDate,  createFinancialSummary as performFinancialSummary,  calculateWithValidation as performCalculationWithValidation} from '../../utils/lunarGrid/financialCalculations';
...
export const useLunarGridCalculations = () => {
   ...
   */
     const calculateDailyBalances = useCallback((     startingBalance: number,     transactionsByDate: Map<string, Transaction[]>   ): BalanceCalculation[] => {     return performDailyBalances(startingBalance, transactionsByDate);   }, []);
   ...
   */
     const recalculateFromDate = useCallback((     existingCalculations: BalanceCalculation[],     fromDate: string,     updatedTransactions: Map<string, Transaction[]>   ): BalanceCalculation[] => {     return performRecalculateFromDate(existingCalculations, fromDate, updatedTransactions);   }, []);

  /**
   * Creează breakdown detaliat pentru o anumită zi
   * Oferă separare clară între disponibil, economii și patrimoniu total
   */
  const calculateBreakdown = useCallback((calculation: BalanceCalculation): TransactionBreakdown => {
    return {
   ...
   */
     const calculateWithValidation = useCallback((     startingBalance: number,     transactionsByDate: Map<string, Transaction[]>   ): CalculationResult => {     return performCalculationWithValidation(startingBalance, transactionsByDate);   }, []);

  /**
   * Creează sumar financiar pentru o perioadă
   * Oferă overview rapid: venituri, cheltuieli, economii, schimbare netă
   */
     const createFinancialSummary = useCallback((     calculations: BalanceCalculation[],     startingBalance: number   ): FinancialSummary => {     return performFinancialSummary(calculations, startingBalance);   }, []);
  ...
  const findCalculationDifferences = useCallback((
    ...
    for (const [date, newCalc] of newMap) {
      ...
      if (Math.abs(oldCalc.availableBalance - newCalc.availableBalance) > 0.01) {
        differences.push(date);
      }
    }
  ...
  const validateCalculationConsistency = useCallback((
    ...
    for (let i = 1; i < calculations.length; i++) {
      ...
      const totalDailyChange = curr.breakdown.dailyIncome 
