...
export interface UseRecurringTransactionsResult {
  ...
  // Template operations
  createTemplate: (templateData: Partial<RecurringTemplate>) => Promise<RecurringTemplate>;
  updateTemplate: (id: string, updates: Partial<RecurringTemplate>) => Promise<RecurringTemplate>;
  deleteTemplate: (id: string) => Promise<void>;
  validateTemplate: (template: Partial<RecurringTemplate>) => TemplateValidationResult;
  ...
  // Bulk operations
  bulkUpdateTemplates: (operation: BulkUpdateOperation) => Promise<RecurringTemplate[]>;
...
export function useRecurringTransactions(
  ...
  const generationConfig = useMemo((): GenerationConfig => ({
  ...
  const generationStats = useMemo(() => ({
    ...
    lastGenerationTime: lastGenerationResult?.statistics.generationTimeMs || 0
  }), [templates.length, activeTemplates.length, generatedTransactions.length, conflicts.totalConflicts, lastGenerationResult]);
  ...
  const updateTemplate = useCallback(async (
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Template update failed';
      setErrors(prev => [...prev, errorMessage]);
      throw error;
    }
  ...
  const deleteTemplate = useCallback(async (id: string): Promise<void> => {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Template deletion failed';
      setErrors(prev => [...prev, errorMessage]);
      throw error;
    }
  }, [templates, config.mockMode]);
  ...
  const resolveAllConflicts = useCallback((): GeneratedTransaction[] => {
    ...
    const resolved = resolveConflicts(generatedTransactions, config.existingTransactions);
  ...
  const bulkUpdateTemplates = useCallback(async (
