...
const Checkbox = forwardRef<HTMLInputElement, CheckboxProps>(({ 
  ...
  return (
    <div className={cn(
      ...
      labelPosition === 'left' ? 'flex-row-reverse justify-between' : 'flex-row',
      'flex items-center',
      wrapperClassName
    )}>
      <input
        ...
        disabled={disabled}
        data-testid={dataTestId || `checkbox-${checkboxVariant}${error ? '-error' : ''}${disabled ? '-disabled' : ''}`}
        {...rest}
      />
      {labelText && (
        <label 
      ...
      {error && (
        <div className="text-sm text-red-600 mt-1 w-full">
          {error}
        </div>
      )}
    </div>
