...
describe("Checkbox", () => {
  ...
  it("se randează corect cu atributele de bază", () => {
    ...
    expect(screen.getByTestId("test-checkbox")).toHaveAttribute(
      "type",
      "checkbox",
    );
  });
  ...
  it("afișează mesajul de eroare", () => {
    render(
      <Checkbox
        label={TEST_CONSTANTS.CHECKBOX.LABEL}
        error={TEST_CONSTANTS.CHECKBOX.REQUIRED_ERROR}
      />,
    );
    expect(
      screen.getByText(TEST_CONSTANTS.CHECKBOX.REQUIRED_ERROR),
    ).toBeInTheDocument();
  });
  ...
  it("respectă proprietatea checked pentru starea inițială", () => {
    render(
      <Checkbox checked data-testid="test-checkbox" onChange={() => {}} />,
    );
