...
const Spinner: React.FC<SpinnerProps> = ({
  ...
  const getSpeedClass = () => {
    switch (speed) {
      case "slow":
        return "animate-spin"; // Could be customized with slower animation
      case "fast":
        return "animate-spin"; // Could be customized with faster animation
      case "normal":
      default:
        return "animate-spin";
    }
  ...
  return (
    ...
    >
      <svg
        ...
        className={cn(loader({ size, color }), getSpeedClass())}
      >
        {showBackground && (
          <circle
            cx="22"
