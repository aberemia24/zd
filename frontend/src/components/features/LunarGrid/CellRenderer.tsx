...
export const CellRenderer: React.FC<CellRendererProps> = ({
  ...
  const cellClassName = useMemo(() => {
    ...
    const typeClasses = {
      [TransactionType.INCOME]:
        amount > 0 ? "text-green-700 font-medium" : "text-gray-500",
      [TransactionType.EXPENSE]:
        amount > 0 ? "text-red-700 font-medium" : "text-gray-500",
      [TransactionType.SAVING]:
        amount > 0 ? "text-blue-700 font-medium" : "text-gray-500",
    };
  ...
  return (
    ...
    >
      <span className="block w-full px-1">{formattedAmount}</span>
    </td>
  );
};
...
export interface UseCellStateOptions {
  ...
  onSelectionChange?: (
    selected: Array<{ category: string; subcategory?: string; day: number }>,
  ) => void;

  /** Callback pentru intrarea în modul editare */
  onEditingChange?: (
    editing: { category: string; subcategory?: string; day: number } | null,
  ) => void;
}

export const useCellState = (options: UseCellStateOptions = {}) => {
  const [selectedCells, setSelectedCells] = useState<
    Array<{ category: string; subcategory?: string; day: number }>
  >([]);
  ...
  const handleCellSelect = useCallback(
    (category: string, subcategory: string | undefined, day: number) => {
      ...
      setSelectedCells((prev) => {
        ...
        if (isSelected) {
          newSelection = prev.filter(
            (cell) =>
              `${cell.category}-${cell.subcategory || ""}-${cell.day}` !==
              cellId,
          );
        } else {
