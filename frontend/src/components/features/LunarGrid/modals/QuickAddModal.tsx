        ...
        <div className={transactionModalBody()}>
          ...
          <div>
            ...
            />
                    ...
                    {/* Recurring Checkbox */}          <div>            <Checkbox              label="Tranzacție recurentă"              checked={form.data.recurring}              onChange={(e: React.ChangeEvent<HTMLInputElement>) => {                const checked = e.target.checked;                form.updateData({ recurring: checked });                setShowRecurringOptions(checked);                if (!checked) {                  form.updateData({ frequency: undefined });                }              }}              data-testid="quick-add-recurring-checkbox"            />          </div>          {/* Recurring Options */}          {showRecurringOptions && (            <div className="pl-6 border-l-2 border-blue-200">              <Select                label="Frecvență"                value={form.data.frequency || ''}                onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>                   form.updateData({ frequency: e.target.value })                }                options={frequencyOptions}                error={validation.errors.frequency}                data-testid="quick-add-frequency-select"                placeholder="Selectează frecvența"              />            </div>          )}
            ...
            <div className="bg-slate-50 p-3 rounded-md text-sm">
              <div className="flex justify-between items-center">
                ...
                <span className={`font-semibold ${
                  financialImpact.isPositive ? 'text-emerald-600' : 'text-red-600'
                }`}>
                  {financialImpact.isPositive ? '+' : ''}{financialImpact.amount} RON
                </span>
              </div>
            </div>
          )}
