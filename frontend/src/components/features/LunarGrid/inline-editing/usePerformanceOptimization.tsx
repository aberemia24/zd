...
export const usePerformanceOptimization = ({
  ...
  const setupEventDelegation = useCallback(() => {
    ...
    const handleClick = (e: Event) => {
      ...
      if (cellElement) {
        ...
        const cellId = cellElement.getAttribute('data-testid')?.replace('editable-cell-', '');
        if (cellId) {
          // Emit custom event pentru cell click
          gridElement.dispatchEvent(new CustomEvent('cell-click', {
    ...
    const handleDoubleClick = (e: Event) => {
      ...
      if (cellElement) {
        const cellId = cellElement.getAttribute('data-testid')?.replace('editable-cell-', '');
        if (cellId) {
          gridElement.dispatchEvent(new CustomEvent('cell-double-click', {
            detail: { cellId, originalEvent: e }
          }));
        }
      }
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.target && gridElement.contains(e.target as Node)) {
        const cellElement = (e.target as HTMLElement).closest('[data-testid^="editable-cell"]');
        
        if (cellElement) {
          const cellId = cellElement.getAttribute('data-testid')?.replace('editable-cell-', '');
          if (cellId) {
            gridElement.dispatchEvent(new CustomEvent('cell-keydown', {
              detail: { cellId, originalEvent: e }
            }));
          }
        }
  ...
  const measurePerformance = useCallback((operation: string, fn: () => void) => {
    ...
    if (duration > 16) {
      console.warn(`Performance warning: ${operation} took ${duration.toFixed(2)}ms (target: <16ms)`);
    } else {
