...
import { useInlineCellEdit } from '../../components/features/LunarGrid/inline-editing/useInlineCellEdit';
...
describe('useInlineCellEdit - Phase 1 Core Functionality', () => {
  ...
  test('inițializează hook-ul corect cu date live', () => {
    const { result } = renderHook(
      () => useInlineCellEdit(createMockProps()),
      { wrapper: TestWrapper }
    );
  ...
  test('validează input real-time cu valori live', async () => {
    ...
    expect(mockOnSave).toHaveBeenCalledWith(150.50); // onSave primește number
    ...
    expect(result.current.error).toContain(EXCEL_GRID.INLINE_EDITING.VALIDATION_ERRORS.INVALID_NUMBER);
  });
  ...
  test('integrare cu shared-constants pentru diferite tipuri de validare', async () => {
    ...
    expect(amountResult.current.error).toContain(EXCEL_GRID.INLINE_EDITING.VALIDATION_ERRORS.NEGATIVE_VALUE);

    // Test percentage validation
    const { result: percentResult } = renderHook(
      () => useInlineCellEdit(createMockProps({ validationType: 'percentage', initialValue: 50 })),
      { wrapper: TestWrapper }
    );
    ...
    expect(percentResult.current.error).toContain(EXCEL_GRID.INLINE_EDITING.VALIDATION_ERRORS.PERCENTAGE_RANGE);
  });
  ...
  test('concurrent editing prevention during save', async () => {
    ...
    const slowOnSave = jest.fn(() => new Promise(resolve => {
      resolveSave = resolve;
    }));
