...
import { EditableCell } from '../../components/features/LunarGrid/inline-editing/EditableCell';
...
describe('EditableCell - Phase 1 UI Component', () => {
  ...
  test('salvează cu Enter în modul editare', async () => {
    ...
    render(
      <TestWrapper>
        <EditableCell {...createMockProps({ isEditing: true, value: 300.50 })} />
      </TestWrapper>
    );
  ...
  test('arată eroarea de validare cu stiluri CVA', () => {
    const errorMessage = EXCEL_GRID.INLINE_EDITING.VALIDATION_ERRORS.INVALID_NUMBER;
  ...
  test('integrare cu shared-constants pentru placeholder-uri', () => {
    ...
    const input = screen.getByPlaceholderText(EXCEL_GRID.INLINE_EDITING.PLACEHOLDER.AMOUNT);
    expect(input).toBeInTheDocument();
  });
